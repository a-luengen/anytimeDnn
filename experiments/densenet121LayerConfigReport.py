...
from context import densenet, utils, data_loader, data_utils
...
if __name__ == "__main__":
    ...
    for skip_layers in range(1, 7):
        for run in range(0, 30):
            ...
            layer_config = []
            if isinstance(policy, densenet.DropPolicies.DenseNetDropNPolicy):
                layer_config = policy.getFullConfig()
    ...
    df.to_csv(os.path.join(STORE_RESULT_PATH, f'{name}_experiment_results.csv'), index=False)